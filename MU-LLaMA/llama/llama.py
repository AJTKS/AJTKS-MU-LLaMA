...
class ModelArgs:
    ...
    norm_eps: float = 1e-5
    rope_theta: float = 500000 # llama3
...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0):
    ...
    t = torch.arange(end, device=freqs.device, dtype=torch.float32) # llama3
    freqs = torch.outer(t, freqs) # llama3
...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        #model_parallel_size = fs_init.get_model_parallel_world_size() # llama3
